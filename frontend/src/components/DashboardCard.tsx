...
export function GreetingCardContent({
    userName,
    deckCount,
    cardCount,
}: {
...
export function StudyProgressCardContent({
    dueCards,
    totalCards,
}: {
    dueCards: number;
    ...
    const progressPercentage =
        totalCards > 0 ? 100 - Math.round((dueCards / totalCards) * 100) : 100;
    return (
        <div>
...
export function PieChartCardContent({
    ...
    data: { new: number; learning: number; mastered: number };
}) {
    const chartData = [
...
export function QuickActionsContent() {
    ...
    return (
        <div className="quick-action-buttons">
            ...
            <button className="quick-action-btn" onClick={() => navigate('/add-card')}>
                Add New Card
            </button>
            <button className="quick-action-btn" onClick={() => navigate('/decks')}>
                View Decks
            </button>
            <button className="quick-action-btn" onClick={() => navigate('/card-builder')}>
                Card Builder
            </button>
        </div>
    );
